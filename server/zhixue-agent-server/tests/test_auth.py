...
def _register(client, nickname="小明", grade="大二"):
    response = client.post("/api/v1/auth/register", json={"nickname": nickname, "grade": grade})
...
def test_register_rejects_blank_and_overlong_nickname(client):
    assert client.post("/api/v1/auth/register", json={"nickname": "   "}).status_code == 400
    assert client.post("/api/v1/auth/register", json={"nickname": "x" * 17}).status_code == 400
    assert client.post("/api/v1/auth/register", json={}).status_code == 400


def test_register_rejects_unknown_grade(client):
    response = client.post("/api/v1/auth/register", json={"nickname": "小明", "grade": "博后"})
...
def test_auth_endpoints_require_contract_version_header_to_match(client):
    ...
    response = client.post("/api/v1/auth/register", json={"nickname": "小明"},
...
def test_session_expiry_is_enforced(tmp_path):
    ...
    outcome = auth.register_user(repository, "小明")
