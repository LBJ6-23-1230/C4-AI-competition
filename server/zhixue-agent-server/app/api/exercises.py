...
def submit_exercises(set_id: str):
	...
	if (not isinstance(user_id, str) or not user_id.strip()
		...
		return jsonify({"errorCode": "BAD_REQUEST", "message": "userId, idempotencyKey and answers are required", "details": None}), 400
		...
		if profile_model_dict:
			if response["needReplan"]:
				...
				if plan is not None:
					...
					history.update({"adjustmentReason": history["reason"], "triggerEvidence": history["evidenceIds"]})
					_repository.save("plan_histories", new_plan.plan_id, history)
