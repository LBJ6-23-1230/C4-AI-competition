...
def append_history(user_input: str, bot_response: str) -> None:
    history = get_history()
    history.append({
...
def _format_history(num_entries: int = 10) -> str:
    lines = []
    for entry in get_history()[-num_entries:]:
        lines.append(
...
def _context_prompt(user_message: str, data: dict[str, Any]) -> str:
    return (
        f"对话历史：{_format_history()}\n"
        f"用户输入：{user_message}\n"
...
def _partner_prompt(user_message: str, data: dict[str, Any]) -> str:
...
# --------------------------------------------------------------------------- 子 Agent
def _handle_query_tasks(message: str, data: dict[str, Any]) -> str:
    return _call_llm(_load_prompt("QueryTasksPrompt.txt"), _context_prompt(message, data))


def _handle_analyze_weakness(message: str, data: dict[str, Any]) -> str:
    return _call_llm(_load_prompt("AnalyzeWeaknessPrompt.txt"), _context_prompt(message, data))


def _handle_get_suggestion(message: str, data: dict[str, Any]) -> str:
    return _call_llm(_load_prompt("GetSuggestionPrompt.txt"), _context_prompt(message, data))


def _handle_match_partner(message: str, data: dict[str, Any]) -> str:
    return _call_llm(_load_prompt("MatchPartnerPrompt.txt"), _partner_prompt(message, data))


def _handle_update_profile(message: str, data: dict[str, Any]) -> str:
    raw = _call_llm(_load_prompt("UpdateProfilePrompt.txt"), _context_prompt(message, data),
                    response_json=True)
    return str((extract_json(raw) or {}).get("reply") or "已帮你更新信息。")


def _handle_analyze_wrong(message: str, data: dict[str, Any], image_base64: str | None) -> str:
    if image_base64:
        ...
        return _call_llm_with_image(system_prompt, _context_prompt(message, data), image_base64)

    raw = _call_llm(_load_prompt("AnalyzeWrongPrompt.txt"), _context_prompt(message, data),
                    response_json=True)
    parsed = extract_json(raw) or {}
    ...
    if isinstance(analysis, dict) and analysis:
        ...
        analysis.setdefault("userId", "demo-user")
        ...
        if isinstance(questions, list):
            ...
            (_CHAT_DIR / "mock_wrong_questions.json").write_text(
...
def chat(message: str, image_base64: str | None = None,
         frontend_data: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    intent, llm_used = detect_intent(message)

    reply = ""
    if llm_used:
        try:
            if intent == "analyze_wrong":
                reply = _handle_analyze_wrong(message, data, image_base64)
            else:
