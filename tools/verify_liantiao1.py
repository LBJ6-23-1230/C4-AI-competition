...
# 前端专属检查的标签前缀集合。
#
# 为什么需要：本脚本同时校验前端与后端，但检查结果都归在同一个
# "static-consistency" 组里。当脚本在「后端独立交付包」中运行时（entry/ 不存在），
# 需要准确报出「跳过了几项前端检查」——靠这个集合去 RESULTS 里数，
# 比手工维护计数器可靠（手工计数器容易在增删检查时忘记同步）。
FRONTEND_LABELS: tuple[str, ...] = (
    "前端存在 ApiDefaults 单一真源文件",
    "ApiDefaults 契约版本与后端一致",
    "页面中无硬编码后端 IP（应统一引用 ApiDefaults）",
    "ApiModels 契约版本 = v0.3",
    "前端 DEMO_USER_ID = demo-user",
    "游客 userId 无散落硬编码（应统一引用 ApiDefaults）",
...
def run_static_consistency() -> None:
    ...
    # 前端缺席时的处理
    #
    # 为什么需要：本脚本同时校验前端与后端。当它被放进「后端独立交付包」
    # 运行时，前端目录（entry/）并不存在，前端相关的静态检查会全部 FAILED，
    # 让后端同学误以为是自己改坏了代码。
    #
    # 正确做法：显式识别「前端缺席」这一状态，把前端检查整组标记为 SKIP，
    # 而后端相关的检查（契约鉴权声明等）**照常执行**。
    # ------------------------------------------------------------------
    frontend_present: bool = ets_root.exists()
    # 前端检查项数直接由标签表长度决定，增删检查时只需改 FRONTEND_LABELS
    frontend_checks_total: int = len(FRONTEND_LABELS)
    if not frontend_present:
        print("   [SKIP] 未检测到前端目录 entry/ —— 判定为「后端独立包」运行模式。")
        print("          前端相关的 %d 项静态一致性检查整组跳过（不属后端职责），"
              % frontend_checks_total)
        print("          后端相关的检查（契约鉴权声明等）照常执行。")
    ...
    if frontend_present:
        ...
        api_models = ets_root / "api" / "ApiModels.ets"
        ...
        pages = (ROOT / "entry" / "src" / "main" / "resources" / "base"
    ...
    check(group, "后端鉴权接口源码齐备（app/api/auth.py）",
          (SERVER_DIR / "app" / "api" / "auth.py").exists(), True)
    check(group, "后端鉴权测试套件齐备（tests/test_auth.py）",
          (SERVER_DIR / "tests" / "test_auth.py").exists(), True)
    ...
    if not frontend_present:
        note(group, "前端静态检查跳过数（后端独立包模式）", len(FRONTEND_LABELS))
