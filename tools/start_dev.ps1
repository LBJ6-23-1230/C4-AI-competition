...
if (-not $python) {
    ...
    Write-Host '      uv venv .venv-lt --python 3.12' -ForegroundColor Yellow
    Write-Host '      uv pip install --python .venv-lt\Scripts\python.exe flask flask-cors openai python-dotenv pytest' -ForegroundColor Yellow
    Write-Host '    （在工程根目录执行；脚本也会自动尝试 py -3.12 / python）' -ForegroundColor DarkGray
    exit 1
}
