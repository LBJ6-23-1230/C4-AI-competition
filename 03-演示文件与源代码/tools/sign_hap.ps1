...
# ⚠️ AGC 下载下来的 profile 文件名是 `zhixue-release-profileRelease.p7b`（长名），
# 磁盘上**没有** `zhixue-release.p7b`。原先这里写死了短名，于是脚本在下面
# 那个"输入文件检查"里就直接 `[!!] 输入文件缺失，终止` —— 从来没有真正跑起来过。
# 现在两个名字都试，取存在的那个（短名优先保持向后兼容）。
$profile = @(
    (Join-Path $signDir 'zhixue-release.p7b'),
    (Join-Path $signDir 'zhixue-release-profileRelease.p7b')
) | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
if (-not $profile) { $profile = Join-Path $signDir 'zhixue-release-profileRelease.p7b' }
