...
def test_image_routes_to_multimodal_handler_when_llm_ready(monkeypatch):
    ...
    def _fake_with_image(system_prompt, user_prompt, image_base64, response_json=False):
        calls.append({"image": image_base64, "user_prompt": user_prompt})
        # 图片分支现在要 **JSON**：答复文本照旧，但知识点必须**结构化**带出去，
        # 否则前端拿不到它去请求对应题集（"拍错题 → 针对性地练"这条闭环接不上）。
        return json.dumps({
            "reply": "已识别：二叉树后序遍历",
            "knowledgePoint": "二叉树后序遍历",
            "wrongType": "顺序记反",
        }, ensure_ascii=False)
    ...
    # 结构化知识点必须一路带出到 chat() 的返回值（供 API 层下发）
    assert result["wrongAnalysis"]["knowledgePoint"] == "二叉树后序遍历"
    assert result["wrongAnalysis"]["wrongType"] == "顺序记反"
