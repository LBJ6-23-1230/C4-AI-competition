# -*- coding: utf-8 -*-
"""liantiao5 一键联调：自己拉起后端进程 → 实跑 HTTP 验证 → 归档证据 → 关停。

用法::

    python integration/run_liantiao5.py                     # 默认端口 5000
    python integration/run_liantiao5.py --port 5057
    python integration/run_liantiao5.py --no-boot --base http://127.0.0.1:5000
...
def main():
    ...
    if args.no_boot or args.base:
        ...
        # 对话历史（`chat/history.json`）也要重定向 —— 它原先**没被重定向**，
        # 于是每跑一次联调就往交付包的历史文件里塞进 probe-a / probe-b 分桶。
        # 实测证据：队友交付的 history.json 累积到 18,162 B / 5 个分桶
        # （demo-user / user-a / user-b / probe-a / probe-b），干净状态只需 demo-user。
        chat_dir = WORKSPACE_TMP / f"chat-{args.port}"
        if chat_dir.exists():
            shutil.rmtree(chat_dir, ignore_errors=True)
        chat_dir.mkdir(parents=True, exist_ok=True)
        env["ZHIXUE_CHAT_DIR"] = str(chat_dir)
        log(f"      联调临时数据 {scratch.relative_to(ROOT)} + {chat_dir.relative_to(ROOT)}"
            f"（交付包 repository.json / history.json / mock_wrong_questions.json 均保持不变）")
